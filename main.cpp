...
public:
    void render_frame(cv::Mat &frame); //just display the frame with optional help messages
    ...
    void draw_help(cv::Mat &frame); //draw the help text on the frame
    void toggle_help(); //turn on and off the visual help text
...
{
    name.assign(name_in); //assign the new string
    cv::namedWindow(name,1);
}

void visualize::render_frame(cv::Mat &frame)
{
    if (help){
        draw_help(frame);
    }
...
void visualize::draw_help(cv::Mat &frame)
{
    //if (help){
...
//the new, cleaned out main function
int main(int argc, char* argv[])
{
    ...
    while(cap.isOpened() && keyresponse!='q'){ //to keep things simple, let's just have this as the calibration loop
        //figure out what to do in this loop
        keyresponse=cv::waitKey(1);
        
        switch (keyresponse){
            //case 'q': check the quit case above
                //break;
            case 'h':
